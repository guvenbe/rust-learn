...
impl TemperatureSensor {
    pub fn get_average_temperature(&self) -> Option<f64> {
        (!self.readings.is_empty()).then_some(self.readings.iter().copied().sum::<f64>()/self.readings.len() as f64)
    }
    pub fn record_temperature(&mut self, temp: f64) {
    ...
    pub fn get_max_temperature(&self) -> Option<f64> {
        (!self.readings.is_empty()).then_some(
            self.readings.iter().copied().fold(f64::NEG_INFINITY, |a,b|a.max(b)),
        )
    }
}
...
mod tests {
    ...
    trait TemperatureSensorTestExt {
        fn with_temps(temp: &[f64]) -> TemperatureSensor;
    }
    ...
    #[test]
    fn max_temp_returns_none_when_two_readings_are_present() {
