...
fn main() {
    ...
    let object = create_paintable_trait_object(true);
...
fn paint_red3(object: &dyn Paint) {
    object.paint("red".to_owned())
}
//when we don't know the concrete return type
fn create_paintable_trait_object(vehicle: bool) -> Box<dyn Paint> {
    if vehicle {
        Box::new(Car {
            info: VehicleInfo {
                make: "Honda".to_owned(),
